...
exports.yelpRating = function(req, res) {
  ...
  async.parallel({
    ...
    demand: function(callback) {
      var request = require("request");
      var async = require("async");

      var location = req.body.location;
      var cuisine = req.body.domain;
      console.log(location);
      console.log(cuisine);
    ...
    safety: function(callback) {
      ...
      var renderData =  {
        ...
        tweets: results.tweetFinder,
        total_score: (results.competition.competition + parseFloat(results.cost)+ parseFloat(results.demand)+ parseFloat(results.safety))/4
      }
